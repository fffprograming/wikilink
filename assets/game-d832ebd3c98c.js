...
if (typeof document !== "undefined") {
  ...
  function showHomeContent(visible) {
    const content = document.querySelector('.static');
  ...
  const initialMode = window.WLG_START_MODE;
  delete window.WLG_START_MODE;
  if (rseed) startRandom(rseed);
  else if (initialMode === "daily") {
    const today = jstToday();
    const done = lsGet(dailyKey(today));
    if (done) dailyResult(today, done.g, done.m); else startDaily();
  }
  else if (initialMode === "random") startRandom();
  else if (initialMode === "sudden") startSudden();
  else home();
}
